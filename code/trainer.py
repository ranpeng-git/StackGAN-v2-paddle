...
def KL_loss(mu, logvar):
    ...
    KLD_element = mu.pow(2).add(logvar.exp())*(-1).add(1).add(logvar)
...
def zeros_(x):
    ...
    temp_value = paddle.create_parameter(shape=x.shape, dtype=x.dtype,is_bias=False, default_initializer=initializer)
    x.set_value(temp_value)


@paddle.no_grad()
def fill_(x , value):
    initializer = init.Constant(value=value)
    temp_value = paddle.create_parameter(shape=x.shape, dtype=x.dtype,is_bias=False, default_initializer=initializer)
    x.set_value(temp_value)

@paddle.no_grad()
def normal_(x ,mean,std):
    initializer = init.Normal(mean=mean,std=std)
    temp_value = paddle.create_parameter(shape=x.shape, dtype=x.dtype,is_bias=False, default_initializer=initializer)
    x.set_value(temp_value)
...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:
        nn.init.orthogonal(m.weight.detach(), 1.0)

    elif classname.find('BatchNorm') != -1:
        m.weight =  normal_(m.weight.detach() , 1.0 , 0.02)
        m.bias = fill_(m.bias.detach() ,0)
...
def load_params(model, new_param):
    for p, new_p in zip(model.parameters(), new_param):
        p = paddle.create_parameter(shape=new_p.shape , dtype= str(new_p.numpy().dtype) , default_initializer= paddle.nn.initializer.Assign(new_p))
...
def load_network(gpus):
    ...
    for i in range(len(netsD)):
        ...
        netsD[i] = paddle.DataParallel(netsD[i])
        # print(netsD[i])
    ...
    if cfg.TRAIN.NET_D != '':
        for i in range(len(netsD)):
            ...
            state_dict = paddle.load('%s%d.pdparams' % (cfg.TRAIN.NET_D, i))
...
def define_optimizers(netG, netsD):
    ...
    for i in range(num_Ds):
        opt = optim.Adam(parameters = netsD[i].parameters(),
                         learning_rate =cfg.TRAIN.DISCRIMINATOR_LR,
                         beta1=0.5,
                         beta2=0.999)
    ...
    optimizerG = optim.Adam(parameters = netG.parameters(),
                            learning_rate=cfg.TRAIN.GENERATOR_LR,
                            beta1=0.5,
                            beta2=0.999)
...
def save_model(netG, avg_param_G, netsD, epoch, model_dir):
    ...
    paddle.save(
        netG.state_dict(),
        '%s/netG_%d.pdparams' % (model_dir, epoch))
    for i in range(len(netsD)):
        netD = netsD[i]
        paddle.save(
            netD.state_dict(),
            '%s/netD%d.pdparams' % (model_dir, i))
...
class GANTrainer(object):
    ...
    def train_Dnet(self, idx, count):
        flag = count % 100
        batch_size = self.real_imgs[0].shape[0]
    ...
    def train_Gnet(self, count):
        ...
        for i in range(self.num_Ds):
            ...
            if flag == 0:
                summary_G = summary.scalar('G_loss%d' % i, errG.detach().numpy()[0])
    ...
    def train(self):
        ...
        real_labels = paddle.ones(shape=[self.batch_size,])
        self.real_labels = paddle.create_parameter(shape = real_labels.shape , 
                                                    dtype = 'float32',
                                                    default_initializer= paddle.nn.initializer.Assign(real_labels))

        fake_labels = paddle.zeros(shape=[self.batch_size ,])
        self.fake_labels = paddle.create_parameter(shape = fake_labels.shape , 
                                                    dtype = 'float32',
                                                    default_initializer= paddle.nn.initializer.Assign(fake_labels))
        nz = cfg.GAN.Z_DIM
        noise = paddle.create_parameter(shape = [self.batch_size ,nz] , 
                                        dtype = 'float32',
                                        default_initializer= paddle.nn.initializer.Assign(paddle.randn([self.batch_size ,nz]))
        )
                ...
                if step == 0:
                    print('''[%d/%d][%d/%d] Loss_D: %.2f Loss_G: %.2f'''
                           % (epoch, self.max_epoch, step, self.num_batches,
                              errD_total.detach().numpy()[0], errG_total.detach().numpy()[0]))
                count = count + 1
    ...
    def save_superimages(self, images, folder, startID, imsize):
        ...
        vutils.save_image(images.detach(), fullpath, normalize=True)
    ...
    def save_singleimages(self, images, folder, startID, imsize):
        for i in range(images.shape[0]):
            ...
            img = (images[i] + 1.0) / 2
            img = images[i].add(1).divide(2)*(255).clip(0, 255).astype('uint8')
            # range from [0, 1] to [0, 255]
            ndarr = img.transpose(perm = [1, 2, 0]).detach().cpu().numpy()
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            netG = paddle.DataParallel(netG)
            print(netG)
            # tate_dict = paddle.load(cfg.TRAIN.NET_G)
            state_dict = \
...
class condGANTrainer(object):
    ...
    def train(self):
        ...
        self.gradient_half = paddle.to_tensor([0.5])
    ...
    def save_superimages(self, images_list, filenames,
        ...
        for i in range(batch_size):
            ...
            for j in range(num_sentences):
                ...
                # break
            super_img = paddle.concat(super_img, 0)
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            noise = paddle.create_parameter(shape = [self.batch_size ,nz] , 
                                        dtype = 'float32',
                                        default_initializer= paddle.randn([self.batch_size, nz]))
            # if cfg.CUDA:
            #     netG.cuda()
            #     noise = noise.cuda()
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                embedding_dim = t_embeddings.shape[1]
                batch_size = imgs[0].shape[0]
                # noise.data.resize_(batch_size, nz)
                noise = noise.detach().reshape([batch_size, nz])
                # noise.data.normal_(0, 1)
                noise = noise.detach().normal(0,1)
                ...
                for i in range(embedding_dim):
                    ...
                    if cfg.TEST.B_EXAMPLE:
                        ...
                        fake_img_list.append(fake_imgs[2].detach().cpu())
                    else:
